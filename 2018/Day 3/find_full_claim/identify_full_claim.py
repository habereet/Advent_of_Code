...
def get_claim_data(line, pattern):
	...
	thisClaim = claim(claimMatch.group(1), claimMatch.group(2), claimMatch.group(3), claimMatch.group(4), claimMatch.group(5), (int(claimMatch.group(4)) * int(claimMatch.group(5))))
	return thisClaim
...
claim = namedtuple('claim', 'claimNumber fromLeft fromTop width heigth area')
pattern = re.compile('\#(\d*) \@ (\d*),(\d*)\: (\d*)x(\d*)')
claimList = []

for currentClaim in read_file("../claims_data.txt"):
...
fullClaim = ""
for currentClaim in list(claimList):
	if currentClaim.area == sum(row.count(currentClaim.claimNumber) for row in cloth):
		fullClaim = currentClaim.claimNumber
		break
print(f'\nclaim with no competing claims: {fullClaim}')